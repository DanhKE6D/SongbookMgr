...
class RfbProto {
  ...
  RfbProto(String h, int p, int timeout) throws IOException{
  	//- viewer = v;
    host = h;
    port = p;
  ...
  */
  ...
  synchronized void writeKeyEvent(int keySym, int metaState, boolean down) throws IOException {
	eventBufLen = 0;
	//Log.i(TAG,"WriteKeyEvent: keySym  = " + keySym + " metaState = " + metaState + " down = " + (down ? "TRUE" : "FALSE"));
